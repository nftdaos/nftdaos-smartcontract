...
contract("Mint", function (accounts) {
  ...
  it("proposalERC20Spend", async function () {
    ...
    await vaultFactory.mint('XXX', 'XXX', [testERC721.address], ['1'], web3.utils.toWei('1', 'ether'), web3.utils.toWei('0', 'ether'), web3.utils.toWei('1', 'ether'), 7);
   ...
   await tokenVaultStaking.convertFTokenToVeToken(web3.utils.toWei('0.8', 'ether'));
    ...
    console.log("await tokenVaultStaking.getVotes(accounts[0])" + (await tokenVaultStaking.getVotes(accounts[0])).toString());
     console.log(" tokenVaultStaking.address"+tokenVaultStaking.address.toString());
     console.log(" tokenVault.address"+tokenVault.address.toString());
   rs = await tokenVaultGovernor.propose(
    ...
    console.log(
     ...
     assert.equal(await testERC20.balanceOf(tokenVault.address), 0);
     assert.equal(await testERC20.balanceOf(accounts[0]),  web3.utils.toWei('1', 'ether'));
  });
  ...
  it("proposalERC721Spend", async function () {
   ...
   rs = await tokenVaultGovernor.propose(
    ...
    console.log(
     ...
     assert.equal((await testERC721New.ownerOf('1')).toString(),  accounts[0].toString());
  ...
  it("proposalERC1155Spend", async function () {
    //
   await vaultFactory.mint('XXX', 'XXX', [testERC721.address], ['3'], web3.utils.toWei('1', 'ether'), web3.utils.toWei('0', 'ether'), web3.utils.toWei('1', 'ether'), 7);
     let rs = await vaultFactory.vaults('0');
    let tokenVault = await TokenVault.at(rs);
    ...
    let callData = web3.eth.abi.encodeFunctionCall(
     ...
     console.log("await tokenVaultStaking.getVotes(accounts[0]) 2 " + (await tokenVaultStaking.getVotes(accounts[0])).toString());
      console.log(" tokenVaultStaking.address 2"+tokenVaultStaking.address.toString());
       console.log(" tokenVault.address 2"+tokenVault.address.toString());
       await tokenVaultStaking.convertFTokenToVeToken(web3.utils.toWei('0.8', 'ether'));
    await tokenVaultStaking.delegate(accounts[0]);
   ...
   rs = await tokenVaultGovernor.propose(
    ...
    assert.equal((await testERC1155.balanceOf(tokenVault.address, '1')).toString(),"0");
     assert.equal((await testERC1155.balanceOf(accounts[0], '1')).toString(), "1000");
