...
contract("Mint", function (accounts) {
  ...
  it(" treasury flow ", async function () {
    ...
    await weth.deposit({ from: accounts[0],value: web3.utils.toWei('0.01', 'ether') });
    await weth.transfer(tokenVaultExchange.address, web3.utils.toWei('0.01', 'ether'));
    await tokenVault.transfer(tokenVaultExchange.address, web3.utils.toWei('0.1', 'ether'), { from: accounts[0] });
    await tokenVaultExchange.shareExchangeFeeRewardToken();
    //assert.equal((await  tokenVault.balanceOf(tokenVaultTreasury.address)).toString(), web3.utils.toWei('0.02', 'ether').toString());
    //assert.equal((await   weth.balanceOf(tokenVaultTreasury.address)).toString(), web3.utils.toWei('0.002', 'ether').toString());
    console.log('tokenVault.balanceOf tokenVaultTreasury', (await tokenVault.balanceOf(tokenVaultTreasury.address)).toString());
