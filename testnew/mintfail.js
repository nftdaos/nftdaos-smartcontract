...
contract("Mint", function (accounts) {
  ...
  beforeEach('setup contract', async () => {
    ...
    testERC721B = await TestERC721.new('ZZZ', 'ZZZ');
    mockGovernor = await MockGovernor.new();
    mockGovernorB = await MockGovernor.new();

    testERC721.mint(accounts[0], '1', '');
    testERC721A.mint(accounts[0], '1', '');
    testERC721B.mint(accounts[0], '1', '');
    ...
    await testERC721B.setApprovalForAll(vaultFactory.address, true);
    settings.setGovernorSetting(testERC721.address, mockGovernor.address, 5, 5);
    settings.setGovernorSetting(testERC721B.address, mockGovernorB.address, 5, 5);

  });

  it("mint vault amount params.listprice = 0 fail", async function () {
    await truffleAssert.reverts(
      vaultFactory.mint('XXX', 'XXX', [testERC721.address], ['1'], web3.utils.toWei('1000000', 'ether'), web3.utils.toWei('100000', 'ether'), 0, 20),
    );
  ...
  it("mint vault amount params.supply = 0 fail", async function () {
    ...
    await truffleAssert.reverts(
      vaultFactory.mint('XXX', 'XXX', [testERC721.address], ['1'], 0, 0, web3.utils.toWei('100000', 'ether'), 20),
    );
  ...
  it("revert min govern invalid setting(has govenor + non governor)", async function () {

    await truffleAssert.reverts(
      vaultFactory.mint('XXX', 'XXX', [testERC721.address, testERC721A.address], ['1', '1'], web3.utils.toWei('1000000', 'ether'), web3.utils.toWei('100000', 'ether'), web3.utils.toWei('1', 'ether'), 20),
    );

  });

  it("revert min govern invalid setting(has two  different governor)", async function () {

    await truffleAssert.reverts(
      vaultFactory.mint('XXX', 'XXX', [testERC721.address, testERC721B.address], ['1', '1'], web3.utils.toWei('1000000', 'ether'), web3.utils.toWei('100000', 'ether'), web3.utils.toWei('1', 'ether'), 20),
    );
