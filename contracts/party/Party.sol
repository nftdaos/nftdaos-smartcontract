...
import {ReentrancyGuardUpgradeable} from "../libraries/openzeppelin/upgradeable/security/ReentrancyGuardUpgradeable.sol";
...
import {IERC721Metadata} from "../libraries/openzeppelin/token/ERC721/IERC721Metadata.sol";
//import {IERC20} from "../ibraries/openzeppelin/token/ERC20/IERC20.sol";
...
contract Party is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable {
    ...
    IVaultFactory public immutable iVaultFactory;
    ...
    constructor(
        ...
        address _iVaultFactory,
        address _weth
    ) {
        partyFactory = msg.sender;
        partyDAOMultisig = _partyDAOMultisig;
        iVaultFactory = IVaultFactory(_iVaultFactory);
        weth = IWETH(_weth);
    }
    ...
    function _fractionalizeNFT(uint256 _amountSpent) internal {
        ...
        nftContract.approve(address(iVaultFactory), tokenId);
        ...
        _tokens[0] = address(nftContract);
        uint256[] memory _ids = new uint256[](1);
        _ids[0] = tokenId;
        uint256 vaultNumber = iVaultFactory.mint(
            name,
            symbol,
           _tokens,
            _ids,
       ...
       // store token vault address to storage
        tokenVault = ITokenVault(iVaultFactory.vaults(vaultNumber));
